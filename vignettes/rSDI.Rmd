...
# Hocam SDI fonksiyonu değil SDI_MODIFIED üzerinde çalışıyorum SDI kaybolmasın diye.
# Bu nedenle bu örneği SDI_MODIFIED ile ekledim. directionality argümanını henüz eklemedim

toyGraphWithSDI <- SDI_MODIFIED(toyGraph)
edge_attr(toyGraphWithSDI)
vertex_attr(toyGraphWithSDI)
```
...
```{r echo=T, eval=F}
plotSDI(TMSDI, variant="vuw", circle.size.scale=1.5)
```
...
plotSDI(TMSDI, variant=c("viw","vow"))
```
