...
SDI(TurkiyeMigration.flows, TurkiyeMigration.nodes) %>% plotSDI()
```
...
rSDI plotting functions make use of available open map packages in the R ecosystem to make a geographical plot of SDI measurements:

```{r echo=T, eval=F}
require(rnaturalearth)
plot.SDI <- function(g, variant=NULL, circle.size.scale=1) {
  lay <- create_layout(g, 'manual', x=V(g)$longitude, y=V(g)$latitude)
  world <- ne_countries(scale = "medium", returnclass = "sf")
  z<-get.vertex.attribute(g,paste0("SDI_",variant))
  #normal size is normalized to mean 10 (pt)
  normalsize <- 10*z/mean(z,na.rm=TRUE)
  p <- ggraph(lay) +
    geom_sf(data=world) + 
    coord_sf(xlim = c(min(V(g)$latitude), max(V(g)$latitude)), ylim = c(min(V(g)$longitude),max(V(g)$longitude))) +
    geom_node_point(size = normalsize*circle.size.scale, aes(color="red"),alpha=0.4)+
...
```{r echo=T, eval=F}
plot.SDI(TMSDI, variant="vuw", edges=TRUE)
```
...
Burada Middle Earth örneği yapılabilir. Ayrıca TurkeyMigration için default'tan farklı bir harita nasıl kullanılır anlatılabilir. Ancak ikisi farklı. Birincide imajı ve nasıl hizalanacağını vermek lazım. İkincisi ise bir sd nesnesi olur tahmin ediyorum.
